...
def get_data(data_path: str, json_path: str,  meta_path: str):
    ...
    for k, v in to_filter.items():
      if type(v[0]) is not float:
          adata = adata[adata.obs[k].isin(list(to_filter[k])),:]
      else:
          adata = adata[adata.obs[k] > v[0]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corr_meta', type=str, required=True,
                        help='correct meta-data')
    ...
    adata = get_data(args.data, args.cells_to_filter, args.corr_meta)
